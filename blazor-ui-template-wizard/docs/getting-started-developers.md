...
launcher. Select `Telerik UI for Blazor Template Wizard: Launch` and press `Enter` to launch the extension
...
`blazorvscode.vsix` is the default extensionName.
...
After installation, use `ctrl+shift+p (Windows)` or `cmd+shift+p (Mac)` to open the Extension Launcher and select `Telerik UI for Blazor Template Wizard: Launch` to run the extension.
